...
if ( have_rows( 'carousel_slides' ) ) :
	...
	harmonium_enqueue_slick_scripts();
	...
	while ( have_rows( 'carousel_slides' ) ) :
		...
		$classname     = $color && harmonium_has_array_key( 'color_picker', $color ) ? 'has-font-color color-' . esc_attr( $color['color_picker'] ) : '';
		...
		?>
			<div class="slide-content container <?php echo esc_attr( $classname ); ?>" data-animation="<?php echo esc_attr( harmonium_get_animation_class( array( 'options' => get_sub_field( 'animation' ) ) ) ); ?>">
