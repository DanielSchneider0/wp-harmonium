...
harmonium_display_block_options(
	array(
		...
		'class'     => 'rev-Row content-block accordion-block' . esc_attr( $alignment . $classes ), // Container class.
	)
);
						...
						<?php foreach ( $accordion_items as $accordion_item ) : ?>
							...
							<div class="accordion-item">
								<div class="accordion-item-header">
									<h3 class="accordion-item-title"><?php echo esc_html( $accordion_item['accordion_title'] ); ?>
										<button class="accordion-item-toggle" aria-expanded="false" aria-controls="<?php echo esc_attr( $item_content_id ); ?>">
											<span class="screen-reader-text"><?php echo sprintf( esc_html( 'Toggle %s', 'harmonium' ), esc_html( $accordion_item['accordion_title'] ) ); ?></span>
								...
								<div id="<?php echo esc_attr( $item_content_id ); ?>" class="accordion-item-content" aria-hidden="true">
									<?php echo harmonium_get_the_content( $accordion_item['accordion_text'] ); // phpcs: xss: ok. ?>
								</div>
							</div>
						<?php endforeach; ?>
					</div>
